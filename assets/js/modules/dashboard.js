...
export const dashboard = {
    ...
    _cacheElements() {
        this.elements = { 
            ...
            backgroundLayer: document.querySelector('#view-dashboard .dashboard-background-layer'),
    ...
    initBarChart() {
        ...
        this.barChart = new Chart(ctx, {
            ...
            options: {
                ...
                indexAxis: 'x',
                scales: {
                     x: { grid: { color: 'rgba(255, 255, 255, 0.2)', lineWidth: 1 }, ticks: { color: 'white', font: { size: 14, weight: 'bold' } } },
                    xTop: { position: 'top', grid: { drawOnChartArea: false, color: 'rgba(255, 255, 255, 0.2)' }, ticks: { color: 'white', font: { size: 12, weight: 'normal' } } },
                    y: { display: true, min: 0, max: 100, grid: { display: true, color: 'rgba(255, 255, 255, 0.2)', lineWidth: 1 }, ticks: { display: false } }
                },
                plugins: { 
    ...
    applySettings: function() {
        ...
        if (this.elements.backgroundLayer) {
            this.elements.backgroundLayer.style.backgroundImage = (showMainBg && mainBackground && mainBackground.value) ? `url('${mainBackground.value}')` : 'none';
        }

        if (this.elements.mainContainer && dashboardMargins) {
            this.elements.mainContainer.style.setProperty('--padding-top', `${dashboardMargins.top}px`);
            this.elements.mainContainer.style.setProperty('--padding-right', `${dashboardMargins.right}px`);
            this.elements.mainContainer.style.setProperty('--padding-bottom', `${dashboardMargins.bottom}px`);
            this.elements.mainContainer.style.setProperty('--padding-left', `${dashboardMargins.left}px`);
        }
        ...
        if(operationModes && this.elements.operationLights) {
        ...
        if(this.barChart && barChartStyling) {
            ...
            chartOptions.scales.x.grid.color = barChartStyling.gridColor || 'rgba(255, 255, 255, 0.2)';
            chartOptions.scales.x.grid.lineWidth = barChartStyling.gridLineWidth ?? 1;
            chartOptions.scales.y.grid.color = barChartStyling.gridColor || 'rgba(255, 255, 255, 0.2)';
            chartOptions.scales.y.grid.lineWidth = barChartStyling.gridLineWidth ?? 1;
            ...
            chartOptions.scales.xTop.ticks.font.weight = barChartStyling.rangeFontWeight || 'normal';
    ...
    updateDisplay: function(data, error = null) {
        ...
        if (this.barChart && data) {
            ...
            displayValues.forEach((val, i) => {
                ...
                const fontWeight = styling.valueFontWeight || 'normal';
                valueDiv.innerHTML = `<span style="font-weight: ${fontWeight}; font-size: ${styling.valueFontSize}px; color: ${styling.valueTextColor};">${val.toFixed(2)}</span> <span style="font-size: ${styling.unitFontSize}px; color: ${styling.unitTextColor};">${unit}</span>`;
        ...
        if(operationModes && data && this.elements.operationLights) {
            ...
            const activeModeName = matchedMode ? matchedMode.name : null;
